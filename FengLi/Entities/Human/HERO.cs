...
public partial class HERO : Photon.MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (LevelInfo.getInfo(FengGameManagerMKII.Level).horse && IN_GAME_MAIN_CAMERA.GameType == GameType.Multiplayer && photonView.IsMine)
        {
        ...
        if (Application.platform == RuntimePlatform.WindowsWebPlayer || Application.platform == RuntimePlatform.OSXWebPlayer)
        {
            ...
            if (string.Compare(Application.absoluteURL, "http://fenglee.com/game/aog/aog1.unity3d", ignoreCase: true) != 0)
            {
                flag = true;
            }
            if (flag)
    ...
    public bool IsGrounded()
    {
        ...
        return Physics.Raycast(layerMask: ((LayerMask)((int)layerMask2 | (int)layerMask)).value, origin: base.gameObject.transform.position + Vector3.up * 0.1f, direction: -Vector3.up, distance: 0.3f);
    }
    ...
    private bool IsFrontGrounded()
    {
        LayerMask layerMask = 1 << LayerMask.NameToLayer("Ground");
        LayerMask layerMask2 = 1 << LayerMask.NameToLayer("EnemyBox");
        return Physics.Raycast(layerMask: ((LayerMask)((int)layerMask2 | (int)layerMask)).value, origin: base.gameObject.transform.position + base.gameObject.transform.up * 1f, direction: base.gameObject.transform.forward, distance: 1f);
    }
    ...
    private bool IsUpFrontGrounded()
    {
        LayerMask layerMask = 1 << LayerMask.NameToLayer("Ground");
        LayerMask layerMask2 = 1 << LayerMask.NameToLayer("EnemyBox");
        return Physics.Raycast(layerMask: ((LayerMask)((int)layerMask2 | (int)layerMask)).value, origin: base.gameObject.transform.position + base.gameObject.transform.up * 3f, direction: base.gameObject.transform.forward, distance: 1.2f);
    }
        ...
        else
        {
                    ...
                    if (inputManager.isInputDown[InputCode.attack1])
                    {
                        ...
                        else
                        {
                            ...
                            else if (skillId == "levi")
                            {
                                ...
                                LayerMask layerMask = 1 << LayerMask.NameToLayer("Ground");
                                LayerMask layerMask2 = 1 << LayerMask.NameToLayer("EnemyBox");
                                if (Physics.Raycast(ray, out var hitInfo, 1E+07f, ((LayerMask)((int)layerMask2 | (int)layerMask)).value))
                                {
                                    if ((bool)bulletRight)
                                    {
                            ...
                            else if (skillId == "petra")
                            {
                                ...
                                if (Physics.Raycast(ray2, out var hitInfo2, 1E+07f, ((LayerMask)((int)layerMask4 | (int)layerMask3)).value))
                                {
                                    if ((bool)bulletRight)
                                    {
                                        bulletRight.GetComponent<Bullet>().disable();
            ...
            if (inputManager.isInput[InputCode.rightRope] && ((!base.animation.IsPlaying("attack3_1") && !base.animation.IsPlaying("attack5") && !base.animation.IsPlaying("special_petra")) || state == HERO_STATE.Idle))
            {
                ...
                else
                {
                    ...
                    if (Physics.Raycast(ray5, out var hitInfo5, 10000f, ((LayerMask)((int)layerMask10 | (int)layerMask9)).value))
                    {
            ...
            if (inputManager.isInput[InputCode.bothRope] && ((!base.animation.IsPlaying("attack3_1") && !base.animation.IsPlaying("attack5") && !base.animation.IsPlaying("special_petra")) || state == HERO_STATE.Idle))
            {
                ...
                if (!bulletLeft && !bulletRight)
                {
                    ...
                    if (Physics.Raycast(ray6, out var hitInfo6, 1000000f, ((LayerMask)((int)layerMask12 | (int)layerMask11)).value))
                    {
    ...
    private void showAimUI()
    {
        ...
        if (!Physics.Raycast(ray, out var hitInfo, 1E+07f, layerMask3.value))
        {
            return;
        }
        ...
        vector4 *= 1000000f;
        vector5 *= 1000000f;
        if (Physics.Linecast(base.transform.position + vector2, base.transform.position + vector2 + vector4, out var hitInfo2, layerMask3.value))
        {
    ...
    private void FixedUpdate()
    {
        ...
        if (flag2 && flag3)
        {
            ...
            Vector3 vector8 = Vector3.RotateTowards(current2, base.rigidbody.velocity, 1.53938043f * num12, 1.53938043f * num12);
            vector8.Normalize();
            spinning = true;
        ...
        else if (flag2)
        {
            ...
            Vector3 vector9 = Vector3.RotateTowards(current3, base.rigidbody.velocity, 1.53938043f * num14, 1.53938043f * num14);
            vector9.Normalize();
            spinning = true;
        ...
        else if (flag3)
        {
            ...
            Vector3 vector10 = Vector3.RotateTowards(current4, base.rigidbody.velocity, 1.53938043f * num16, 1.53938043f * num16);
            vector10.Normalize();
            spinning = true;
    ...
    public void PauseAnimation()
    {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType != 0 && photonView.IsMine)
        {
    ...
    private void suicide()
    {
        if (IN_GAME_MAIN_CAMERA.GameType != 0)
        {
            netDie(base.rigidbody.velocity * 50f, isBite: false, -1, string.Empty);
