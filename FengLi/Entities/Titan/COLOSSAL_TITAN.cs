...
public class COLOSSAL_TITAN : Photon.MonoBehaviour
{
	...
	private void Start()
	{
		...
		door_broken.SetActiveRecursively(state: false);
		door_closed.SetActiveRecursively(state: true);
	}
	...
	public void update()
	{
		...
		else if (state == "kick")
		{
			...
			{
				attackChkOnce = true;
				door_broken.SetActiveRecursively(state: true);
				door_closed.SetActiveRecursively(state: false);
				if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && !FengGameManagerMKII.LAN)
				{
